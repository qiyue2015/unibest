...
const props = withDefaults(defineProps<Props>(), {
  title: '用户隐私保护提示',
  desc: '感谢您使用本应用，您在使用本应用的服务之前，请仔细阅读并同意相关协议。',
  subDesc: `当您点击同意并开始使用产品服务时，即表示您已理解并同意该条款内容，该条款将对您产生法律约束。若您拒绝，将无法使用相应服务。`,
  protocol: '用户隐私保护指引',
})

const title = computed(() => props.title)
const desc = computed(() => props.desc)
const subDesc = computed(() => props.subDesc)
const protocol = computed(() => props.protocol)
...
onBeforeMount(() => {
  ...
  if (wx.onNeedPrivacyAuthorization) {
    ...
    console.warn('wx.onNeedPrivacyAuthorization 不存在，当前环境/基础库不支持')
  }
})
